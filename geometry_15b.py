...
def main():
    ...
    # ========== 1. 骨架几何: 全层激活矩阵 ==========
    print("\n--- [1] 骨架几何 (30段语料 mean-pooled) ---", flush=True)
    acts = []  # (n_layers+1, n_texts, d) 含 embedding
    for t in CORPUS:
        ids = tok(t, return_tensors="pt").to(model.device)
        h = model(**ids, output_hidden_states=True).hidden_states
        acts.append(torch.stack([x[0].mean(0) for x in h]))  # (L+1, d)
    A = torch.stack(acts)  # (n_texts, L+1, d)
    n_texts = A.shape[0]
    Lp1 = A.shape[1]
    print(f"[dims] {n_texts} texts x {Lp1} layers")

    # 相邻层 cos (embed 0 与 layer0 之间也算)
    cos_adj = []
    for l in range(Lp1 - 1):
        c = F.cosine_similarity(A[:, l], A[:, l+1], dim=-1).mean().item()
        cos_adj.append(c)
    print(f"[adj-cos] 均值={np.mean(cos_adj):+.4f} 逐层={['%.3f' % c for c in cos_adj]}", flush=True)
    ...
    for i in range(Lp1):
        for j in range(Lp1):
            if abs(i - j) >= 3:
                c = F.cosine_similarity(A[:, i], A[:, j], dim=-1).mean().item()
                offs.append(abs(c))
    print(f"[off-diag|cos|] 均值={np.mean(offs):.4f}", flush=True)

    # 有效维@90%: 每层激活矩阵 SVD 能量
    print("[eff-dim@90%] ", end="", flush=True)
    for l in [0, 1, 3, 5, 8, 12, 16, 20, 24, n_layers]:
        M = A[:, l].float().cpu().numpy()  # (n_texts, d)
        M = M - M.mean(0, keepdims=True)
        try:
            s = np.linalg.svd(M, compute_uv=False)
            cum = np.cumsum(s**2) / (np.sum(s**2) + 1e-12)
            k = int(np.searchsorted(cum, 0.90) + 1)
            print(f"L{l}:{k}({100.0*k/d_model:.1f}%) ", end="", flush=True)
        except Exception as e:
            print(f"L{l}:ERR ", end="", flush=True)
    print("", flush=True)

    # 范数剖面
    norms = A.norm(dim=-1).mean(0).tolist()
    print(f"[norm-profile] {['%.1f' % n for n in norms]}", flush=True)
    print(f"[norm] 单调递增={all(norms[i] <= norms[i+1]*1.02 for i in range(len(norms)-1))}", flush=True)
    ...
    with torch.no_grad():
        for l in range(n_layers):
            # 前向到层 l, 注入噪声, 继续前向
            # 用 hook 方式: 注册 forward hook 到第 l 层
            noise = torch.randn_like(A[0, l+1].unsqueeze(0)) * 2.0  # (1,d) 近似
            captured = {}
            def make_hook(layer_idx, noise_vec):
                def hook(module, inp, out):
                    h = out[0] if isinstance(out, tuple) else out
                    captured[layer_idx] = h + noise_vec.to(h.dtype)
                    return out
                return hook
            h = model.model.layers[l].register_forward_hook(make_hook(l, noise))
            out = model(**ids, output_hidden_states=False)
            h.remove()
            if l in captured:
                pass
            # 直接用注入后的 hidden 计算 logits 距离
            # 简化: 用隐藏状态扰动代理 — 改测 logit lens 距离
            logits = out.logits[0]
            # 对比 base: 只测最终 logits 的 top1 是否变化
            top1_base = base.argmax(-1)
            top1_new = logits.argmax(-1)
            flip = (top1_base != top1_new).float().mean().item()
            dist = F.cosine_similarity(base.float().reshape(-1, 1), logits.float().reshape(-1, 1)).mean().item()
            sens.append((l, flip, dist))
    sens.sort(key=lambda x: -x[1])
    print("[sensitivity] top8: " + " | ".join(f"L{l}:flip{flip:.2f}/cos{dist:.3f}" for l, flip, dist in sens[:8]), flush=True)
    print(f"[sensitivity] 最钝3层: {', '.join(f'L{l}({flip:.2f})' for l, flip, _ in sens[-3:])}", flush=True)
    ...
    act_norm = A[:, n_layers-1].norm(dim=-1).mean().item()
    q = "什么是人工智能？"
