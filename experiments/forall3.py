...
files=[f for f in files if os.path.basename(f) not in ('runner.py','fastrun.py','forkrun.py','forall.py','forall2.py','forall3.py','say.py','np_tok.py','qwen_np.py')]
...
def crange(*a):
    ...
    return builtins.range(st,min(en,st+sp*CAP),sp)
PF=os.environ.get('PF','/workspace/_p3.json'); RF=os.environ.get('RF','/workspace/_r3.jsonl')
try: i=json.load(open(PF))['i']
except: i=0
BUD=float(os.environ.get('BUD','0.62'))
t0=time.time(); fo=open(RF,'a',encoding='utf-8'); n0=i
while i<len(files) and time.time()-t0<BUD:
    path=files[i]; fd,tmp=tempfile.mkstemp(); pid=os.fork()
    if pid==0:
        try:
    ...
    while True:
        ...
        if p==pid: break
        if time.time()-tt>0.04 or time.time()-t0>BUD:
...
print(f"{n0} -> {i} / {len(files)}  ({time.time()-t0:.2f}s)")
