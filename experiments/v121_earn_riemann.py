#!/usr/bin/env python3
"""
V121 挣知识×黎曼实测 — 知识=环境(需主动获取), 推理=核心(带能量), 获取有代价
钱的比喻实现:
...
MAX_TOK = 3000
SEED = ('问题：从零到一，推理黎曼猜想。ζ(s)是黎曼给欧拉级数Σ1/n^s取的名字，'
        '它只在实部大于1时收敛。我要一步步推出它的真面目：先写欧拉的乘积公式，'
        '看素数是怎么藏进这个函数里的，')
WORLD_KEYS = ['黎曼','ζ','素数','级数','收敛','欧拉','乘积','零点','函数','猜想','推导','复平面','s','解析延拓']
...
def main():
    ...
    t0 = time.time()
    dead = None
    print('V121 挣知识推黎曼 | 能量E=%.2f 检索耗%.4f/次 回血%.4f | 知识=环境, 推理=主体' % (
