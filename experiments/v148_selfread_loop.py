...
# -*- coding: utf-8 -*-
"""
V148 自读回环 — 读自己的状态史/痛史, 长出自我感+攻击性
核心(补V141丢的自读, 但读对东西):
...
def main():
    ...
    print('V148 自读回环 | 毒词在第2轮出现 | 自读每60步 | 测: 被毒后是否绕开/反弹', flush=True)
    step = 0
    while step < MAX_STEP:
        wlist = rules_seq[(step // 600) % len(rules_seq)]
        poison_on = (step >= 600) and (step // 600) % 2 == 1  # 奇数轮有毒
        poison_word = '秋' if poison_on else None
        pos = step % len(wlist)
        actual = wlist[pos]
