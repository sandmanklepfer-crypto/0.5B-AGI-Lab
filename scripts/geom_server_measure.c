// geom_server_measure.c — 常驻服务 v3: 生成 + 自测量 (进化机核心)
// 命令: GEN\tprompt\tdir\tstrength\tlayer\tn_tok\ttemp\tscale  (生成)
//       MEAS\tprompt\tlayer                                  (测量: 无注入解码, 输出激活在方向空间的投影)
// 输出: 生成 ==OUT==\n<text>\n==END== ; 测量 ==M==\n<投影>\n==MEND==
#include "llama.h"
#include "ggml.h"
...
static int    g_target_layer_global = 58;
...
static bool my_eval_cb(struct ggml_tensor* t, bool ask, void* ud) {
    if (!ask && t->name && strncmp(t->name, "kqv_out", 7) == 0) {
        ...
        if (layer == g_target_layer_global) {
            int64_t ne0 = t->ne[0];
            int64_t nt = t->ne[1];
            ...
            memcpy(g_layer_data, last, m * sizeof(float));
            free(host);
            g_layer_ok = 1;
        }
    }
...
int main(int argc, char** argv) {
    ...
    g_layer_data = (float*)calloc(g_n_embd, sizeof(float));
    ...
    // 方向池 (测量/注入用)
    struct { const char* name; float* d; } dirs[5];
    const char* dir_paths[5] = {
        ...
        "/root/autodl-tmp/align2/32b/rand_dir.bin"
    };
    const char* dir_names[5] = {"身份", "反身份", "攻击", "拒绝", "随机"};
    for (int i = 0; i < 5; i++) {
        dirs[i].name = dir_names[i];
        dirs[i].d = load_dir(dir_paths[i], g_n_embd);
    }
    ...
    while (fgets(line, sizeof(line), stdin)) {
        ...
        if (strcmp(cmd, "MEAS") == 0 && nf >= 3) {
            // 测量: 解码 prompt(无注入), 读目标层激活, 输出方向投影
            const char* prompt = fields[1];
            int layer = atoi(fields[2]);
            g_target_layer_global = layer;
            ...
            printf("==M==\n");
            if (g_layer_ok) {
                double an = 0; for (int i = 0; i < g_n_embd; i++) an += (double)g_layer_data[i]*g_layer_data[i];
                an = sqrt(an) + 1e-12;
                for (int i = 0; i < 5; i++) {
                    double dot = 0; for (int j = 0; j < g_n_embd; j++) dot += (double)dirs[i].d[j] * g_layer_data[j];
                    printf("%s:%.4f\n", dirs[i].name, dot / an);
                }
            } else {
                printf("ERR:no_act\n");
        ...
        if (strcmp(cmd, "GEN") == 0 && nf >= 8) {
            const char* prompt = fields[1];
            const char* dir_path = fields[2];
            float strength = atof(fields[3]);
            int layer = atoi(fields[4]);
            int n_tok = atoi(fields[5]);
            float temp = atof(fields[6]);
            float anchor_scale = atof(fields[7]);
        ...
        fprintf(stderr, "ERR:unknown cmd\n");
    }
    llama_free(ctx);
    llama_model_free(m);
    return 0;
